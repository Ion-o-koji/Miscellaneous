...
<div id="fs-overlay" onclick="enterFsFromOverlay()">
  ...
  <div id="page-view" class="page">
    <div class="srow">
      <input type="search" id="q" placeholder="🔍 Search…" oninput="render()" autocomplete="off" />
      <button class="bbtn write-only" id="btn-sel" onclick="toggleBulk()">✓ Select</button>
    </div>
    <div class="sort-row">
      <select class="sort-cat-sel" id="view-cat-filter" onchange="render()"></select>
  ...
  <div id="page-fav" class="page">
    <div class="srow">
      <input type="search" id="fq" placeholder="🔍 Search favourites…" oninput="renderFav()" autocomplete="off" />
    ...
    <div class="study-wrap">
      ...
      <div class="study-controls">
        <div class="study-ctrl-row">
          <select id="study-filter" onchange="initStudy()"></select>
  ...
  <div class="movl" id="user-ovl">
    <div class="mcard">
      ...
      <p style="font-size:.79rem;color:var(--dim);margin-bottom:14px;line-height:1.5">Be careful. You can only recover this if you forgotten, or change this by contacting me here:<br>Email: olsen.porter@missionary.org<br>Messenger: https://m.me/olsen.porter</p>
  ...
  <div class="movl" id="imp-prog-ovl" onclick="if(event.target===this)this.classList.remove('open')">
    <div class="mcard" style="max-width:360px">
      <h3 id="imp-prog-title">📥 Import Progress</h3>
      <div class="imp-prog-card">
  ...
  <div class="movl" id="cfg-ovl" onclick="if(event.target===this)closeCfg()">
    <div class="mcard" style="max-width:520px">
      ...
      <div class="cfg-section">
        ...
        <div class="cfg-row">
          <div class="cfg-row-left">
            <div class="cr-title">Auto-sync interval</div>
          </div>
          <select id="opt-autosync" onchange="applyAutoSync()">
      ...
      <div class="cfg-section">
        ...
        <div class="cfg-row">
          ...
          <span style="font-size:.72rem;color:var(--dim)">olsen.porter@missionary.org<br>https://m.me/100062141886515</span>
        </div>
        <div class="cfg-row">
          <div class="cfg-row-left">
            <div class="cr-title">Khmer Vocabulary</div>
            <div class="cr-sub">Personal learning tool</div>
          </div>
          <span style="font-size:.72rem;color:var(--dim)">v8</span>
        </div>
        <div class="cfg-row">
          <div class="cfg-row-left">
  ...
  `;
    ...
    function isoWeekKey() {
      ...
      var dayOfYear = Math.floor((d.getTime() - startOfYear) / 86400000) + 1;
    ...
    function onSpillClick() {
      if (importState.active || deleteState.active) el('imp-prog-ovl').classList.add('open');
    }
    ...
    function updateDeleteProgress() {
      ...
      var lb = el('imp-label'),
        ...
        det = el('imp-detail'),
        ttl = el('imp-prog-title');
      if (ttl) ttl.textContent = '🗑️ Delete Progress';
      if (lb) lb.textContent = 'Deleting…';
      if (pc) pc.textContent = pct + '%';
      if (bar) bar.style.width = pct + '%';
      if (det) det.textContent = deleteState.done + ' of ' + deleteState.total + ' deleted';
      setStatus('imp', 'Deleting ' + deleteState.done + ' of ' + deleteState.total + '…');
    }
    ...
    function finishEditCat(oldName) {
      ...
      var defIdx = DEFAULT_CATS.indexOf(oldName);
      if (defIdx !== -1) DEFAULT_CATS[defIdx] = newName;
      allRows.forEach(function(r) {
    ...
    function processPendingQueue() {
      ...
      setBusy(1);
      setStatus('spin-dot', 'Saving to sheet…');
      ...
      fetch(url).then(function(r) {
        ...
        if (j.success) {
          ...
          if (pendingQueue.length > 0) {
            setStatus('spin-dot', pendingQueue.length + ' word' + (pendingQueue.length !== 1 ? 's' : '') + ' left to save…');
            queueRunning = false;
    ...
    function saveEdit() {
      ...
      var url = new URL(SCRIPT_URL);
      url.searchParams.set('action', 'updateWord');
      // FIX: pass id so Apps Script can look up by ID (immune to rowIndex staleness)
      ['english', 'khmer', 'romanization', 'notes', 'category', 'rowIndex'].forEach(function(f) {
        url.searchParams.set(f, editRow[f] || '');
      });
      url.searchParams.set('id', editRow.id || '');
      url.searchParams.set('tab', CURRENT_USER || MAIN_USER);
      url.searchParams.set('user', CURRENT_USER || MAIN_USER);
      fetch(url.toString()).then(function(r) {
        ...
        render();
        renderFav();
        var lbl2 = el('edit-lbl'),
          btn2 = el('edit-btn');
        if (lbl2) lbl2.textContent = '\u2713 Saved!';
        if (btn2) btn2.disabled = false;
        toast('Saved!', 'ok');
        setTimeout(function() {
          if (lbl2) lbl2.textContent = '\uD83D\uDCBE Save to Sheet';
          closeSheet();
        }, 1100);
      }).catch(function(err) {
        setEditLoading(false);
        setBusy(-1);
        toast('Save failed, Re-sync and try again: ' + err.message, 'err');
      });
    }
    ...
    var _statsSavedThisSession = false;
    ...
    function renderProgress() {
      ...
      if (hm) {
        ...
        for (var i = 27; i >= 0; i--) {
          ...
          var key = (function(dt) {
              var kh = new Date(dt.getTime() + 7 * 3600000);
              var p = function(x) {
                return x < 10 ? '0' + x : String(x);
              };
              return kh.getUTCFullYear() + '-' + p(kh.getUTCMonth() + 1) + '-' + p(kh.getUTCDate());
            })(d2),
            cnt = dc[key] || 0,
    ...
    function saveUserStatsToSheet() {
      if (_statsSavedThisSession || !SCRIPT_URL || !MAIN_USER) return;
      _statsSavedThisSession = true;
      var total = allRows.length;
      var mastered = 0,
        studied = 0,
        knew = 0,
    ...
    function renderDashboard(stats, awData) {
      ...
      if (cards) cards.innerHTML = [
        ...
        ['Added This Week', stats.wordsThisWeek || 0],
        ['Top Streak 🔥', stats.topStreakUser ? (esc(stats.topStreakUser) + ' (' + (stats.topStreakCount || 0) + ' days)') : '—']
      ].map(function(d) {
        ...
        if (apiData.results?.length) {
          ...
          apiData.results.slice(0, 4).forEach(function(r) {
            ...
            if (c) {
              ...
              html += '</div><button class="trans-add-btn" onclick="addFromTranslation(_dictAddItems[' + _ai + '].en,_dictAddItems[' + _ai + '].kh)" title="Add to vocabulary">➕ Add</button></li>';
            }
          });
          html += '</ul>';
        }
    ...
    (() => {
      const ENTRY = 'Khmer Vocabulary v8',
