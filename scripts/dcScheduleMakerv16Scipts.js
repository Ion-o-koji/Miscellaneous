    ...
    function buildExportHtml() {
      ...
      const css = `
        *{box-sizing:border-box}
        ...
        #mp-text{line-height:1.8;font-size:15px}
        #mp-text.lang-kh{font-size:18px;font-weight:700;line-height:2.2;-webkit-text-stroke:0.45px #111}
      ...
      const mpENsafe = mpEN.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
      const mpKHsafe = mpKH.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
      ...
      const exportedScript = [
        ...
        '  var el=document.getElementById("mp-text");',
        '  el.textContent=(lang==="en"?_mpEN:_mpKH);',
        '  el.className=(lang==="kh"?"lang-kh":"");',
        '}',
        'document.addEventListener("DOMContentLoaded",function(){',
        '  showMPLang("en");',
        '  function wrapMP(node){',
        '    if(node.nodeType===3){',
        '      var t=node.nodeValue;',
        '      if(/(missionary purpose|\\bmp\\b)/i.test(t)){',
        '        var sp=document.createElement("span");',
        '        sp.innerHTML=t.replace(/(missionary purpose|\\bmp\\b)/gi,function(m){',
        ...
        '      Array.from(node.childNodes).forEach(wrapMP);',
        '    }',
        '  }',
        '  wrapMP(document.body);',
...
<div id="mp-overlay" onclick="if(event.target===this)closeMPOverlay()">
  <div id="mp-box">
    ...
    (() => {
      const ENTRY = 'District Council Schedule Maker v16.html',
