      ...
      if (lastVersion !== CURRENT_VERSION) {
        ...
        scriptTags.forEach(function(script) {
          ...
          if (src && src.includes('khmerVocabularyScripts')) {
            window._kvUpdateAvailable = true;
          }
        });
      }
    })();
...
<div id="fs-overlay" onclick="enterFsFromOverlay()">
  ...
  `;
    ...
    if (MAIN_USER) applyUserBadge();

    function showUpdateBanner() {
      if (document.getElementById('kv-update-banner')) return;
      var b = document.createElement('div');
      b.id = 'kv-update-banner';
      b.style.cssText = 'position:fixed;bottom:0;left:0;right:0;z-index:9999;background:rgba(124,58,237,.93);color:#fff;padding:10px 16px;display:flex;align-items:center;justify-content:space-between;font-size:.84rem;gap:10px;backdrop-filter:blur(6px);border-top:1px solid rgba(168,85,247,.5);box-shadow:0 -4px 18px rgba(0,0,0,.45)';
      b.innerHTML =
        '<span>A new version of the app is available.</span>' +
        '<div style="display:flex;gap:8px;flex-shrink:0">' +
        '<button onclick="window.location.reload()" style="background:rgba(255,255,255,.18);border:1px solid rgba(255,255,255,.35);border-radius:20px;color:#fff;padding:5px 14px;cursor:pointer;font-size:.8rem;font-weight:700;white-space:nowrap">Reload Now</button>' +
        '<button onclick="document.getElementById(\'kv-update-banner\').remove()" style="background:transparent;border:1px solid rgba(255,255,255,.25);border-radius:20px;color:rgba(255,255,255,.7);padding:5px 10px;cursor:pointer;font-size:.8rem;white-space:nowrap">Later</button>' +
        '</div>';
    ...
    function init() {
      if (window._kvUpdateAvailable) showUpdateBanner();
      checkStreakReset();
      updateStudyStreak();
      loadSettings();
      refreshAllCatDropdowns();
